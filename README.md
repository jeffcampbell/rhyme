...
*†Gemma 4 26B A4B QAT was run on a local LM Studio server (M-series MacBook) rather than a hosted endpoint, using the 4-bit quantization-aware-trained (QAT) build, with reasoning disabled (`reasoning_effort: none`). Its default thinking mode makes a full local run impractically slow (~90s/retrieval query); this is a quantized, non-thinking configuration, so treat it as directional and distinct from the free-tier Gemma 4 26B A4B IT row above.*
