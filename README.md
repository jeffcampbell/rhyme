...
*§Gemini 3.8 Flash has mandatory reasoning on OpenRouter (`reasoning.mandatory: true`, no `enabled: false` escape hatch) — unlike Qwen3.8 27B above, thinking cannot be turned off, only throttled. Run at `reasoning: {effort: "low"}` rather than the model's default `medium` to keep cost reasonable; treat this row as a low-effort-reasoning configuration rather than strictly comparable to models left at default reasoning (Kimi K3, Inkling, Gemini 3.7 Flash).*
